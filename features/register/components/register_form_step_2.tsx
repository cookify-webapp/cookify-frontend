      ...
      {() => (
        <div>
          ...
          <div className="space-y-6 mt-4">
            <RadioInput label="ไม่มีอาการแพ้ใด ๆ" checked={formik.values.isAllergic === false} onClick={() => formik.setFieldValue('isAllergic', false)} />
            <RadioInput label="มีอาการแพ้อาหารบางชนิด" checked={formik.values.isAllergic} onClick={() => formik.setFieldValue('isAllergic', true)} />
          </div>
          {
              ...
              <div className="mt-6">
                ...
                <div className="w-full lg:w-[300px] mt-6">
                  <PrimaryButton 
                    title="เลือกวัตถุดิบ" 
                    onClick={() => {
                      const tempSelectedIngredients = _.cloneDeep(formik.values?.ingredients)
                      ingredientSelectionModalContext.openModal(
                        () => {
                          formik.setFieldValue('ingredients', ingredientSelectionModalContext.selectedIngredients)
                        },
                        () => {
                          ingredientSelectionModalContext.setValue('selectedIngredients', tempSelectedIngredients)
                        }
                      )
                    }} />
                </div>
                ...
                {_.size(formik.values?.ingredients) > 0 && (
                  <>
                    {_.map(formik.values?.ingredients, (ingredient, index) => (
                      <div
